...
{
  wchar_t* p = path;
  if (*path && (!*title || title[1] != ':'))
  { // not absolute device path
    if (title[0] == '\\' || title[0] == '/')
    { // absolute path
      if (p[0] && p[1] == ':')
        p += 2;
...
void DebugLog(int indent, const char* format, ...)
{
  ...
  {
    int bfirst;
    if (api.GetInt(L"", L"NoLog", 0) == 0)   // TODO:   Hack here as VER_REVISION is undeclared so set == 0 instead.
    {
      s_log = kLogDisabled;
      return;
    }
    ...
    if (bfirst)
    {
      ...
      if (SHGetFolderPath(NULL, CSIDL_DESKTOP, NULL, SHGFP_TYPE_CURRENT, u.path) == S_OK)
      {
        ...
        if (s_log->fp)
        { // don't really care if the log couldn't be made
  ...
  if (*format)
  {
    ...
    for (indent = s_log->indent - (indent > 0 ? indent : 0); indent > 0; --indent)
      fputc('	', s_log->fp);
    fprintf(s_log->fp, "%04hu/%02hu/%02hu %02hu:%02hu:%02hu.%03hu UTC [%s", now.wYear, now.wMonth, now.wDay, now.wHour, now.wMinute, now.wSecond, now.wMilliseconds,
            find_titleA(u.buf));
