...
#define CALTODAYTEXTHEIGHT 13
      ...
      if (iMonthsPast)
      {
        ...
        if (stnew.wMonth > 12)
        {
          ++stnew.wYear;
          stnew.wMonth -= 12;
        } // in case iMonthsPast is negative
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, char* lpCmdLine, int nCmdShow)
{
  ...
  (void)hPrevInstance;
  (void)lpCmdLine;
  (void)nCmdShow; // don't warn me about they being unused
  g_instance = hInstance;
  // if (LoadClockAPI(L"T-Clock" ARCH_SUFFIX, &api))
  if (LoadClockAPI(L"T-Clock", &api))  // Removed reference to ARCH_SUFFIX. Windows 11 is always 64 bit.
    return 2;
  if (!CreateCalender(0))
    return 1;
