...
int main(int argc, char* argv[])
{
  ...
  static struct option long_options[] = { // basic
  ...
  const struct help help_info[] = {
    { 0, DH_ARGV_SHORT, "[option] ..." }, { 'h', 0, "" }, { '1', 0, "" }, { 'u', "check|notify|silent", "check for updates\nreturns IDYES(6), IDNO(7) or IDCANCEL(2)" }, { 0 }
  };
  ...
  if (LoadClockAPI(L"T-Clock", &api)) // Removed reference to ARCH_SUFFIX. No meaning in Windows 11.
  {
  ...
  for (;;)
  {
    ...
    switch (opt)
    {
      ...
      case '1':
        ...
        puts("T-Clock Options version XXX");
        return 1;
      case 'u':
      {
