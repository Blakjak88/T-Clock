...
static MessageBoxCustomData update_notify_data_ = { NULL,
                                                    NULL,
                                                    NULL,
                                                    {
                                                        { L"Update", NULL, 0 },
...
static INT_PTR CALLBACK Window_UpdateCheckDlg(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg)
  {
    case WM_INITDIALOG:
    {
      ...
      data->session = WinHttpOpen(L"T-Clock/", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, WINHTTP_FLAG_ASYNC);   // TODO: Fix versioning.
      if (!data->session)
        goto error;
      ...
      if (!data->connection)
        goto error;
      data->request = WinHttpOpenRequest(data->connection, L"GET", urlComp.lpszUrlPath, NULL, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES,
      ...
      if (wParam == 0)
      {
            ...
            *version_text[1]++ = '\0';
            for (idx = 0; idx < 2; ++idx)
            {
              if (!version_str[0])
              {
                ...
                if (version_next[idx] && version_next[idx] < 99) // TODO: fix version numbering
                  version_next[idx] = 99;
        ...
        if (data->update_available || data->type == UPDATE_SHOW)
        {
          if (data->type != UPDATE_SILENT)
          {
            ...
            idx         = version[0] - 99;  // TODO:   Fix versioning
            ...
            msg_pos += swprintf(msg_pos, msg_end - msg_pos, FMT("\u2237 Your version: ") FMT("") FMT("    (%i change(s) %s stable)\n\n"), abs(idx), abovebehind);  // TODO: Fix versioning
            for (idx = 0; idx < 2; ++idx)
            {
              if (version_next[idx])
              {
                msg_pos += swprintf(msg_pos, msg_end - msg_pos, FMT("%s %hs:%*sv%hs#%u\n"), (!idx ? L"\u2282" : L"\u2283"), kVersionType[idx], (!idx ? 9 : 14), L"",
                                    version_str[idx], version[idx]);
